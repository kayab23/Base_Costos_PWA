...
def list_vendedores(q: str = Query('', alias='q'), limit: int = 10, offset: int = 0, user=Depends(get_current_user)) -> List[dict]:
...
def get_vendedor(vendedor_id: int, user=Depends(get_current_user)):
    with connection_scope() as conn:
        cur = conn.cursor()
